# ------------------------------------------------------------------------------
# TSV output from LCA / taxonomy_to_kraken2 outputs
# Full-lineage version: use complete taxonomy string as #OTU_ID
# Part of Tapirs metabarcoding workflow
# ------------------------------------------------------------------------------

import pandas as pd

final_out = pd.DataFrame()
taxonomy  = pd.DataFrame()

pd.set_option("display.max_colwidth", 400)

outfile      = snakemake.output.tsv
samples      = sorted(snakemake.input.lca)
minimum_rank = snakemake.params.lowest_rank
maximum_rank = snakemake.params.highest_rank
rerep_dir    = snakemake.params.rerep_dir   # e.g. "results/tapirs/09_rereplicated"

for sample in samples:
    name        = sample.split(".")[0]
    sample_name = name.split("/")[-1]
    name        = [name.split("/")[-2], name.split("/")[-1]]
    name        = "/".join(name)

    with open(sample, "r") as file:
        count = len([1 for line in open(sample)])
        if count > 1:
            df = pd.read_csv(file.name, sep="\t", header=None, skiprows=1)
            if ";size=" in str(df[0]):
                df[0] = pd.to_numeric(df[0].str.split(r";size=", expand=True)[1])
            else:
                df[0] = 1
        else:
            df = pd.DataFrame([([0]) + (["unidentified"] * 10)])

    ranks       = ["domain", "phylum", "class", "order", "family", "genus", "species"]
    min_rank    = int(ranks.index(minimum_rank))
    max_rank    = int(ranks.index(maximum_rank))
    ranks_upper = ranks[max_rank:]
    ranks_lower = ranks[min_rank + 1:]

    for i in range(min_rank + 4, 10):
        df[i] = "unidentified"

    df = df[df[1].isin(ranks_upper)]

    rank_list = list(df[1])
    for n, i in enumerate(rank_list):
        if i in ranks_lower:
            rank_list[n] = ranks[min_rank]
    df[1] = rank_list

    lis = list(df[min_rank + 3])
    lit = list(df[2])
    for i in range(len(lis)):
        if lis[i] != "unidentified":
            lit[i] = lis[i]
    df[2] = lit

    species_list = sorted(set(df[2][df[2] != "unidentified"]))
    tsv_index    = species_list
    tsv_index.append("unassigned")
    index = pd.Index(tsv_index)

    dfob = pd.DataFrame(columns=[sample_name], index=index)

    df  = df.replace("unidentified", "")
    tdf = pd.concat([df[df.columns[2:10]]], axis=1, sort=False).drop_duplicates(2).sort_values(2).set_index(2)

    for species in species_list:
        reads = df[0][df[2] == species].sum()
        dfob.loc[species] = reads

    assigned_reads   = dfob[sample_name].sum()
    rerep            = rerep_dir + "/" + name + ".rerep.fasta"
    total_reads      = len([1 for line in open(str(rerep)) if line.startswith(">")])
    unassigned_reads = total_reads - assigned_reads
    dfob.loc["unassigned"] = unassigned_reads

    taxonomy  = (pd.concat([taxonomy,  tdf],  axis=0, sort=True)).drop_duplicates().sort_index()
    final_out = (pd.concat([final_out, dfob], axis=1, sort=False)).fillna(0).astype(int).sort_index()

final_out = final_out.reindex(sorted(final_out.columns), axis=1)

tfob = pd.DataFrame(columns=["taxonomy"], index=final_out.index)

for otu in final_out.index:
    if otu != "unassigned":
        ranks_vals = taxonomy[taxonomy.index == otu].to_string(index=False, header=False).split(" ")
        ranks_vals = [x for x in ranks_vals if x != ""]
        if len(ranks_vals) < 7:
            ranks_vals = ranks_vals + ([""] * (7 - len(ranks_vals)))
        tax_add = "; ".join([
            "d__" + ranks_vals[0].replace(" ", ""),
            "p__" + ranks_vals[1],
            "c__" + ranks_vals[2],
            "o__" + ranks_vals[3],
            "f__" + ranks_vals[4],
            "g__" + ranks_vals[5],
            "s__" + ranks_vals[6],
        ])
        tax_add_ranks = [y for y in [x.split("__")[1] for x in tax_add.split("; ")] if y]
        tfob.loc[otu] = "; ".join(tax_add.split("; ")[0:len(tax_add_ranks)])
    else:
        tfob.loc[otu] = "u__unassigned"

final_out = (pd.concat([final_out, tfob], axis=1, sort=False)).fillna(0).sort_index()

final_out.index.name = "#OTU_ID"

df_unassigned = final_out.loc[["unassigned"], :].copy()
final_out     = final_out.drop("unassigned", axis=0)

final_out["full_lineage_id"] = final_out["taxonomy"]
final_out = final_out.set_index("full_lineage_id")
final_out.index.name = "#OTU_ID"
final_out = final_out.drop(columns=["taxonomy"])

str_len = [len(x.split(";")) for x in final_out.index]
final_out["str_len"] = str_len
final_out = final_out.sort_values(["str_len"], ascending=[False])
final_out = final_out.drop(columns=["str_len"])

df_unassigned.index = ["u__unassigned"]
df_unassigned = df_unassigned.drop(columns=["taxonomy"])
final_out = pd.concat([final_out, df_unassigned])

final_out.to_csv(outfile, sep="\t", index=True, header=True)
